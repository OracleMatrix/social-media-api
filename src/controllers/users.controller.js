...
class UserController {
    ...
    async sendProfilePicture(req, res) {
        try {
            ...
            if (!userExist.profilePicture) {
                return res.status(400).send({message: "User does not have a profilePicture"});
            }
            const filePath = path.join(__dirname, '..', '..', 'uploads', userExist.profilePicture);
            if (!fs.existsSync(filePath)) {
                return res.status(404).send({message: "Profile picture does not exist"});
            }
